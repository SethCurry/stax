...
func (sq *SetQuery) QueryPrintings() *PrintingQuery {
	query := (&PrintingClient{config: sq.config}).Query()
...
func (sq *SetQuery) Clone() *SetQuery {
	...
	return &SetQuery{
		...
		withPrintings: sq.withPrintings.Clone(),
...
func (sq *SetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Set, error) {
	...
	if query := sq.withPrintings; query != nil {
		if err := sq.loadPrintings(ctx, query, nodes,
			func(n *Set) { n.Edges.Printings = []*Printing{} },
			func(n *Set, e *Printing) { n.Edges.Printings = append(n.Edges.Printings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SetQuery) loadPrintings(ctx context.Context, query *PrintingQuery, nodes []*Set, init func(*Set), assign func(*Set, *Printing)) error {
	...
	query.Where(predicate.Printing(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(set.PrintingsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.printing_set
		if fk == nil {
			return fmt.Errorf(`foreign-key "printing_set" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "printing_set" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
