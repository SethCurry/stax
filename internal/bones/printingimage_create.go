...
func (pic *PrintingImageCreate) check() error {
	if _, ok := pic.mutation.URL(); !ok {
		return &ValidationError{Name: "url", err: errors.New(`bones: missing required field "PrintingImage.url"`)}
	}
	...
	if _, ok := pic.mutation.ImageType(); !ok {
		return &ValidationError{Name: "image_type", err: errors.New(`bones: missing required field "PrintingImage.image_type"`)}
	}
