...
func (aq *ArtistQuery) QueryPrintings() *PrintingQuery {
	query := (&PrintingClient{config: aq.config}).Query()
...
func (aq *ArtistQuery) Clone() *ArtistQuery {
	...
	return &ArtistQuery{
		...
		withPrintings: aq.withPrintings.Clone(),
...
func (aq *ArtistQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Artist, error) {
	...
	if query := aq.withPrintings; query != nil {
		if err := aq.loadPrintings(ctx, query, nodes,
			func(n *Artist) { n.Edges.Printings = []*Printing{} },
			func(n *Artist, e *Printing) { n.Edges.Printings = append(n.Edges.Printings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ArtistQuery) loadPrintings(ctx context.Context, query *PrintingQuery, nodes []*Artist, init func(*Artist), assign func(*Artist, *Printing)) error {
	...
	query.Where(predicate.Printing(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(artist.PrintingsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.printing_artist
		if fk == nil {
			return fmt.Errorf(`foreign-key "printing_artist" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "printing_artist" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
