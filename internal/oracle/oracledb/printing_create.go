...
func (pc *PrintingCreate) check() error {
	if _, ok := pc.mutation.Rarity(); !ok {
		return &ValidationError{Name: "rarity", err: errors.New(`oracledb: missing required field "Printing.rarity"`)}
	}
	if v, ok := pc.mutation.Rarity(); ok {
		if err := printing.RarityValidator(v); err != nil {
			return &ValidationError{Name: "rarity", err: fmt.Errorf(`oracledb: validator failed for field "Printing.rarity": %w`, err)}
		}
	}
	return nil
}
